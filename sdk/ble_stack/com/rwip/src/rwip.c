...
#if (RC_CALIBRATE && HZ32000)
extern uint8_t  timer_fast_slow_flag;
static uint32_t cal_rw_timer(uint32_t cycles)
{   
    double res_tmp;
    uint32_t cyc_temp;
    extern double rc32k_error_rate;
    
    res_tmp=(double) cycles;
    cyc_temp =(uint32_t)res_tmp*rc32k_error_rate;	
        
    return cyc_temp;
}

static uint32_t cal_rw_timer1(uint32_t cycles)
{   
    double res_tmp;
    uint32_t cyc_temp;
    extern double rc32k_error_rate;  
...
static uint32_t rwip_slot_2_lpcycles(uint32_t slot_cnt)
{
    ...
    #if (RC_CALIBRATE)  
    if(timer_fast_slow_flag == 2)
    {
        lpcycles = cal_rw_timer1(lpcycles);
    }
    #endif 
...
uint8_t rwip_sleep(void)
{
    ...
    do
    {
         ...
         ************************************************************************/    
        rwip_sleep_time.last_time = REG_AHB0_ICU_TIME_COUNT;    
        rwip_sleep_time.sleep_durtion = sleep_duration;
...
uint32_t rwip_sleep_lpcycles_2_us(uint32_t lpcycles)
{
    ...
    #if (RC_CALIBRATE)  
    if(timer_fast_slow_flag == 1)
    {
        res=cal_rw_timer(res);
    }
    #endif
