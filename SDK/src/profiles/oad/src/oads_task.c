                                   ...
                                   ke_task_id_t const src_id)
{
    ...
    if((param->conidx > BLE_CONNECTION_MAX)|| (gapc_get_conhdl(param->conidx) == GAP_INVALID_CONHDL))
    {
        ...
        struct oads_enable_rsp* rsp = KE_MSG_ALLOC(OADS_ENABLE_RSP, src_id,dest_id, oads_enable_rsp);
...
static int oads_ffc1_upd_req_handler(ke_msg_id_t const msgid,
                                            ...
                                            ke_task_id_t const src_id)
{
	uart_printf("oads_ffc1_upd_req_handler \r\n");
...
void oads_task_init(struct ke_task_desc *task_desc)
{
    // Get the address of the environment
    struct oads_env_tag *srv_env = PRF_ENV_GET(OADS, oads);

    task_desc->msg_handler_tab = oads_msg_handler_tab;
    task_desc->msg_cnt         = ARRAY_LEN(oads_msg_handler_tab);
    task_desc->state           = srv_env->state;
    task_desc->idx_max         = OADS_IDX_MAX;
}
