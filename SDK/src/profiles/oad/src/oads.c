...
static uint8_t oads_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct oads_db_cfg* params)
{
    ...
    struct oads_env_tag* oads_env =(struct oads_env_tag* ) ke_malloc(sizeof(struct oads_env_tag), KE_MEM_ATT_DB);
...
uint32_t oad_updating_user_section_pro(void)
{
    ...
    if(bsec.flag_write == 1)
    {
        ...
        for( i=0;i<len/96;i++)
        {
            flash_write(0, wr_addr+i*96, 96, sec_ptr->data+i*96, NULL);	
        }
        if(len%96 >0)
        {
            flash_write(0, wr_addr+i*96, len%96, sec_ptr->data+i*96, NULL);	
        }
    ...
    {	
        ...
        sec_ptr->data_cnt = 0;
/*
        flash_read(0,SEC_BACKUP_OAD_HEADER_FADDR, sizeof(img_hdr_t),(uint8_t *)&ImgHdr, NULL);
        uart_printf("ImgHdr.rom_ver = %x \r\n", ImgHdr.rom_ver);
        uart_printf("ImgHdr.uid = %x \r\n", ImgHdr.uid);
        uart_printf("ImgHdr.ver = %x \r\n", ImgHdr.ver);
        uart_printf("ImgHdr.LEN = %x \r\n", ImgHdr.len);
*/
        for(int i = 0 ;i < 10;i++)
        {
...
static void oadImgIdentifyReq(uint16_t connHandle, img_hdr_t *pImgHdr)
{
    ...
    noti[0] = pImgHdr->ver&0xff;
    noti[1] = (pImgHdr->ver >> 8) & 0xFF;

    noti[2] = pImgHdr->len&0xff;
    noti[3] = (pImgHdr->len >> 8) & 0xFF;

    rom_ver[0] = pImgHdr->rom_ver&0xff;
    rom_ver[1] = (pImgHdr->rom_ver >> 8) & 0xFF;
