...
const struct attm_desc hids_att_db[HOGPD_IDX_NB] =
{
    ...
    [HOGPD_IDX_REPORT_MAP_VAL]                  = {ATT_CHAR_REPORT_MAP, PERM(RD, ENABLE)|PERM(RP,AUTH), PERM(RI, ENABLE), HOGPD_REPORT_MAP_MAX_LEN},
   ...
   // [HOGPD_IDX_REPORT_VAL]                      = {ATT_CHAR_REPORT, /*PERM(RD, ENABLE)*/PERM(NTF, ENABLE), PERM(RI, ENABLE), HOGPD_REPORT_MAX_LEN},
        ...
        for (report_idx = 0; report_idx < params->cfg[svc_idx].report_nb; report_idx++)
        {
            ...
            switch (params->cfg[svc_idx].report_char_cfg[report_idx] & HOGPD_CFG_REPORT_FEAT)
            {
                ...
                case HOGPD_CFG_REPORT_IN:
                {
                    ...
                    if(mouse_en)
                    {
                        perm = PERM(NTF, ENABLE)|PERM(RD, ENABLE)| PERM(WRITE_REQ, ENABLE);
                    }
                    else
                    {
                        perm = PERM(NTF, ENABLE)|PERM(RD, ENABLE);
                    }
