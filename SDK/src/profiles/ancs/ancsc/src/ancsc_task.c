    ...
    if(state == ANCSC_IDLE)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                prf_gatt_write_ntf_ind(&ancsc_env->prf_env, conidx, handle, param->ntf_cfg,0);
                ...
                if (param->status == ATT_ERR_NO_ERROR)
                {
                    ...
                    if(ancsc_env->env[conidx]->nb_svc > 0)
                    {
                        status = prf_check_svc_char_validity(ANCS_SOURCE_CHAR_MAX, &(ancsc_env->env[conidx]->ancsc.chars[0]),&(ancsc_ancs_char[0]));
                         uart_printf("a3=%x,status=%x\r\n",ancsc_env->env[conidx]->ancsc.chars[0].prop,status);
                                         ...
                                         ke_task_id_t const src_id)
{
	...
	prf_gatt_write(&(ancsc_env->prf_env), conidx,handle, buf,length,GATTC_WRITE,0);
...
{
    // Get the address of the environment
    struct ancsc_env_tag *srv_env = PRF_ENV_GET(ANCSC, ancsc);
