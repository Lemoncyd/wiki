...
#define I2C_DEFAULT_CLK            80000000
...
STATUS i2c_write(uint8_t devAddr, uint8_t addr, uint8_t*buf, uint8_t size)
{
    ...
    #if 0   //Abel modify to remove gcc compile warning
    ...
    #else
	p_i2c_msg.work_mode    = 1;
	p_i2c_msg.work_mode   &= (~I2C_MSG_WORK_MODE_RW_BIT);  // write
    p_i2c_msg.work_mode   &= (~I2C_MSG_WORK_MODE_MS_BIT);  // write
	p_i2c_msg.work_mode   &= (~I2C_MSG_WORK_MODE_AL_BIT);  // 7bit address
	p_i2c_msg.work_mode   |= ( I2C_MSG_WORK_MODE_IA_BIT); // without inner address
    #endif
...
/*    uart_printf("d1=%2x",p_i2c_msg.data);
...
void i2c_init(uint32_t slaveAddr, uint32_t baudRate)
{
    ...
    else
    {
        ...
        REG_APB4_I2C_STAT = 0x000000c0;       // 0100 0000; RXINT_MODE = 0x01, slvstop_stre_scl_en = 0x0
    }
    ...
    SYS_REG0X10_INT_EN |= (0x01 << POS_SYS_REG0X10_INT_EN_I2C);

    if(I2C_MODE==0)
    {
        p_i2c_slave_msg.work_mode = 3;
        p_i2c_slave_msg.send_addr    = slaveAddr;                      // Destiny slave address
    	p_i2c_slave_msg.inner_addr   = 0xff;
    	p_i2c_slave_msg.all_data_cnt = 3+1;                         // 8 byte at most
        p_i2c_slave_msg.addr_flg     = 0;
        p_i2c_slave_msg.trans_done   = 0;
        p_i2c_slave_msg.ack_check    = 1;
        p_i2c_slave_msg.current_data_cnt = 0;
        p_i2c_slave_msg.errno        = 0;



        i2c_msg_init(&p_i2c_slave_msg);
    }
    else
    {
        i2c_msg_reset();
...
static void i2c_send_addr(void)
{
	...
	else //10bit address
	{
		...
		uart_printf("addr10:0x%x",tx_data);
		REG_APB4_I2C_DAT = tx_data;
		i2c_msg->addr_flg ++;
	}
}
...
static void i2c_send_start(void)
{
	...
	cfg_data = REG_APB4_I2C_STAT & 0x0000FF3BUL;
...
void i2c_isr(void)
{
	...
	case 0x00:      // master write
	{
		...
		if (i2c_msg->ack_check && !ack)  // nack
		{
            ...
            if (remain_data_cnt ) // err ,need restart i2c
            {
                i2c_init(I2C_DEFAULT_SLAVE_ADDRESS, 0);
            }
			break;
		...
		if (ucTemp & 0x10)          // all address bytes has been tx, now tx data
		{
			...
			{
			//uart_printf("d:0x%x\n",i2c_msg->data[i2c_msg->current_data_cnt]);
	...
	case 0x02:      // slave write
	{
		...
		if (i2c_stat & 0x0100)      // detect an ACK
		{
			...
			remain_data_cnt --;
			if (remain_data_cnt == 1)
			{
				i2c_msg->trans_done = 1;// TODO
			}
		}
		break;
	}
	...
	case 0x03:      // slave read
	{
		...
		if (i2c_stat & 0x0800)      // match address byte
		{
			...
			if(i2c_slave_addr_cnt >1)
            {
                i2c_msg->work_mode = 2;
                i2c_msg->current_data_cnt = 0;
                REG_APB4_I2C_DAT = i2c_msg->data[i2c_msg->current_data_cnt];
                i2c_msg->current_data_cnt++;
            }
			break;
...
{
//master 读的时候会把当前收到的数返回,第一个数是主地址。
//    uint8_t i;
	if(i2c_msg->trans_done)
	{
		i2c_msg->trans_done = 0;
		//if(i2c_msg.inner_addr == READ_REG_ADDR)
		{
			uart_printf("i2c rx_data_%x  :",i2c_msg->current_data_cnt);
			for(uint8_t i=0; i<i2c_msg->current_data_cnt; i++)
			{
				uart_printf("%x, ", i2c_msg->data[i]);
			}
			uart_printf("\r\n");
            i2c_msg->current_data_cnt =0;
            i2c_slave_addr_cnt = 0;
			i2c_msg->work_mode = 3;
		}
	}
}
...
void test_fram_iic(void)
{
    ...
    else
    {
        while(1)
        {
        	...
        	buffer[2]=i++;
            uart_printf("i2c_w\r\n");
        	i2c_write(0x50,100,buffer,3);
        	Delay_ms(1000);
            uart_printf("i2c_r\r\n");
        	i2c_read(0x50,100,buffer1,3);
        	uart_printf("buffer=%x,%x,%x\r\n",buffer1[0],buffer1[1],buffer1[2]);
