...
#endif
#if (BLE_FEE0_SERVER)
extern const struct prf_task_cbs* fee0s_prf_itf_get(void);
...
#if (BLE_OTAHS_SERVER)
extern const struct prf_task_cbs* otahs_prf_itf_get(void);
#endif // (BLE_OADS_SERVER)
...
#if (BLE_PROV_SERVER)
extern const struct prf_task_cbs* prov_prf_itf_get(void);
#endif
...
static const struct prf_task_cbs * prf_itf_get(uint16_t task_id)
{
    ...
    //uart_printf("~~~~~~~~~~~~~~~~~~~~~~~task id=%d",KE_TYPE_GET(task_id));
    switch(KE_TYPE_GET(task_id))
    {         
        #if (BLE_FEE0_SERVER)
            case TASK_ID_FEE0S:
            prf_cbs = fee0s_prf_itf_get();
            break;
        #endif // (BLE_FEE0_SERVER)
        ...
        #if (BLE_OTAHS_SERVER)
        case TASK_ID_OTAHS:
           	prf_cbs = otahs_prf_itf_get();
         	break;
        #endif // (BLE_OADS_SERVER)
		
		#if (BLE_FTMP_SERVER)
        case TASK_ID_FTMPS:
           	prf_cbs = ftmps_prf_itf_get();
         	break;
        #endif 
...
#if(BLE_CENTRAL)
void prf_reset(uint8_t conidx)
{
    uint8_t i;
    for(i = APPM_SVC_LIST_STOP; i < BLE_NB_PROFILES ; i++)
    {
        if(conidx == prf_env.prf[i].conidx)
        {
            // FW boot profile destruction
            prf_env.prf[i].env  = NULL;
            // unregister profile
...
//	uint8_t idx = KE_IDX_GET(params->app_task);
    uart_printf("prf_add_profile\n");
    if(cbs == NULL)
    {    
            ...
            if(prf_env.prf[i].id == TASK_ID_INVALID)
            {
               // uart_printf("prf_env.prf[%x].task=%x\n",i,prf_env.prf[i].task);
                ...
                if(status == GAP_ERR_NO_ERROR)
                {
                    ...
                    prf_env.prf[i].conidx = KE_IDX_GET(params->app_task);
...
uart_printf("prf_add_profile status=%x\n",status);
    return (status);
}
