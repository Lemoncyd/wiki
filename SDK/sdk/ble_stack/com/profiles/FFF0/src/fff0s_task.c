                                            ...
                                            ke_task_id_t const src_id)
{
    ...
    UART_PRINTF("%s, conidx = %d, src_id - 0x%x\r\n", __func__, param->conidx, src_id);
    ...
    if(state == FFF0S_IDLE)
    {
        ...
        if(fff0s_env->ntf_cfg[conidx] == PRF_CLI_START_NTF)
        {
    		...
    		fff0s_notify_fff1_lvl(conidx, fff0s_env, param);
...
{
    uint8_t conidx = KE_IDX_GET(src_id);
    UART_PRINTF("%s, src_id = 0x%x, conidx = %d, op = 0x%x\r\n", __func__, src_id, conidx, param->operation);
    struct fff0s_env_tag* fff0s_env = PRF_ENV_GET(FFF0S, fff0s);
