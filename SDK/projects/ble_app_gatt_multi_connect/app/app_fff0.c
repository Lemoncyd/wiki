...
void app_fff1_send_lvl(uint8_t conidx, uint8_t* buf, uint8_t len)
{
    ...
    if( ke_state_get(prf_get_task_from_id(TASK_ID_FFF0S))==FFF0S_IDLE )
    {
        ...
        UART_PRINTF("%s, conidx = %d, ntf_cfg = %x\r\n", __func__, conidx, fff0s_env->ntf_cfg[conidx]);
                                  ...
                                  ke_task_id_t const src_id)
{
    if(param->status == GAP_ERR_NO_ERROR)
    {
        ...
        uint8_t buf[128];
        memset(buf, 0xcc, 128);
        app_fff1_send_lvl(param->conidx, buf, 128);
        #else        
...
{
	// Drop the message
	UART_PRINTF("FFF2 conidx = %d, param->value = 0x ", param->conidx);
