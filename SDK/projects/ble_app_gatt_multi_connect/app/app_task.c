                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    if (app_env.conidx != GAP_INVALID_CONIDX)
    {
        ...
        #if (APP_GET_RSSI_EN)
        uart_printf("#####start APP_GET_RSSI_TIMER!\n");
        ke_timer_set(APP_GET_RSSI_TIMER,TASK_APP,300);    ///300 -> 3S
        #endif
	        
		if(app_env.connum < APP_MAX_CON_NUM)
		{
            uart_printf("start adv, connum %d\r\n", app_env.connum);
    	    appm_start_advertising(); 
		}
    }
    else
 ...
 * Others: void
*******************************************************************************/
extern void appm_get_conn_rssi(uint8_t conidx);
static int app_get_rssi_timer_handler(ke_msg_id_t const msgid,      //add 210722
                                    void const *param,
                                    ke_task_id_t const dest_id,
                                    ke_task_id_t const src_id)
{	
	...
	ke_timer_set(APP_GET_RSSI_TIMER,TASK_APP,300);
