...
extern uint8_t cur_adv_data_type;
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    if (app_env.conidx != GAP_INVALID_CONIDX)
    {
        ...
        #else // !(BLE_APP_SEC)
        cfm->auth      = GAP_AUTH_REQ_MITM_NO_BOND;
        #endif // (BLE_APP_SEC)
        ...
        #if 0//(BLE_APP_SEC && !defined(BLE_APP_AM0))
       ...
       // ke_timer_set(APP_GATTC_EXC_MTU_CMD,TASK_APP,20);
		...
		//ke_timer_set(APP_PARAM_UPDATE_REQ_IND,TASK_APP,650);	
                                ...
                                ke_task_id_t const src_id)
{
	...
	switch(param->operation)
	{
    	case (GAPC_UPDATE_PARAMS):  //0x09
    	{
			if (param->status != GAP_ERR_NO_ERROR)
        	{
            	uart_printf("gapc update params fail error=%x !\r\n",param->status);
			}
			else
			{
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if (app_sec_get_bond_status())
    {
        cur_adv_data_type = adv_data_type_sec;
    }
    appm_delete_advertising();
                            ...
                            ke_task_id_t const src_id)
{
    ...
    switch (src_task_id)
    {
        ...
        {
            // Call the Health Thermometer Module
            msg_pol = app_get_handler(&app_otah_handler, msgid, param, src_id);
        } break;
        #endif
