...
uint32_t calc_image_sec_crc(void)
{
	...
	block_total = hdr_back.len % 4;
	if(block_total >0)
	{
		flash_read(data,read_addr,block_total*4);
		calcuCrc = make_crc32(calcuCrc,data,block_total*4);
	}
...
void bim_updata_backup_to_image_sec(void)
{
    ...
    uint32_t backup_size = hdr_back.len * 4+hdr_back.len%4;
