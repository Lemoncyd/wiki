                                ...
                                ke_task_id_t const src_id)
{
    ...
    switch(param->operation)
    {
        ...
        case (GAPM_RESOLV_ADDR):
    	{
    		if(param->status != GAP_ERR_NO_ERROR)
    		{
                uart_printf("GAPM_RESOLV_ADDR FAIL\r\n");
                appm_disconnect();
    		}else
    		{
                uart_printf("GAPM_RESOLV_ADDR OK\r\n");    
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    if (app_env.conidx != GAP_INVALID_CONIDX)
    {
		...
		{
			appm_gapm_resolv_dev_addr(param->peer_addr);
		}
		else
		{
            // Send connection confirmation
                            ...
                            ke_task_id_t const src_id)
{
    ...
    switch (src_task_id)
    {
		...
		case (TASK_ID_FTMPS):
		{
			msg_pol = app_get_handler(&app_ftmp_table_handler, msgid, param, src_id);
		}break;
		#endif
        default:
        {
