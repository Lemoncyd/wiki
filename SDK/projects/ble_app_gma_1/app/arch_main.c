...
void bdaddr_env_init(void)
{
	...
	flash_read(FLASH_SPACE_TYPE_MAIN, 0x27fd0/4, 26, &gma_para[0]);
	if(gma_para[0] != 0xff || gma_para[1] != 0xff || gma_para[2] != 0xff || gma_para[3] != 0xff || gma_para[4] != 0xff || gma_para[5] != 0xff)
	{
        memcpy(ali_para.mac, &gma_para[4], 6);
        ali_para.pid = gma_para[0] | (gma_para[1] << 8) | (gma_para[2] << 16) | (gma_para[3] << 24);

        for(uint8_t i=0;i<16;i++)
            ali_para.secret[i]=gma_para[25-i];
        
        uart_printf("ali_para.mac = ");
        for(uint8_t i = 0; i < 6; i++)
		    uart_printf("%x ", ali_para.mac[i]);
        
		uart_printf("\nali_para.pid = %d\n", ali_para.pid);
        
        uart_printf("ali_para.secret = ");
        for(uint8_t i = 0; i < 16; i++)
		    uart_printf("%x ", ali_para.secret[i]);
        uart_printf("\n");
	}     
    memcpy(&co_default_bdaddr, ali_para.mac, 6);
}
