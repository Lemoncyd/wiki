                                 ...
                                 ke_task_id_t const src_id)
{
    ...
    switch (param->info)
    {
        case (GAPC_PAIRING_SUCCEED):
        {
            ...
            #endif
         /*   #if (BLE_APP_HID)
            ke_timer_set(APP_PARAM_UPDATE_REQ_IND,TASK_APP,30);
            #endif*/
			#if (APP_WHITE_LIST_ENABLE)
			memcpy((uint8_t *)&whitelist_bdaddr,(uint8_t *)gapc_get_bdaddr(0, GAPC_SMP_INFO_PEER),NVDS_LEN_PEER_BD_ADDRESS);
