                               ...
                               ke_task_id_t src_id)
{
    // Counter
    uint8_t counter;
//	uart_printf("app_get_handler msgid:%x\r\n",msgid);
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    if (app_env.conidx != GAP_INVALID_CONIDX)
    {
        ...
        uart_printf("conidx:%d \r\n",app_env.conidx);
        /// Connection handle
        uart_printf("conhdl:%d \r\n",param->conhdl);
        uart_printf("con_interval:%d ,%f\r\n",param->con_interval,param->con_interval * 1.25);
        uart_printf("con_latency:%d \r\n",param->con_latency );
        uart_printf("sup_to:%d \r\n",param->sup_to * 10 );
        uart_printf("peer_addr_type:%d \r\n",param->peer_addr_type);  // 0:ADDR_PUBLIC, 1:ADDR_RAND
        {
        ...
        struct gapc_connection_cfm *cfm = KE_MSG_ALLOC(GAPC_CONNECTION_CFM,
                ...
                gapc_connection_cfm);
        if(param->peer_addr_type== ADDR_PUBLIC)//public
        {
        ...
        if (nvds_put(NVDS_TAG_PEER_ADDR_TYPE, NVDS_LEN_PEER_ADDR_TYPE, (uint8_t *)&param->peer_addr_type) != NVDS_OK)
        {
           ASSERT_ERR(0);
        }

        if (nvds_put(NVDS_TAG_BD_ADDRESS, NVDS_LEN_BD_ADDRESS, (uint8_t *)&co_default_bdaddr) != NVDS_OK)
        {
            ASSERT_ERR(0);
        }
        #if(BLE_APP_SEC)
    ...
    //    ke_timer_set(APP_GATTC_EXC_MTU_CMD,TASK_APP,20);
		...
		ke_timer_set(APP_PARAM_UPDATE_REQ_IND,TASK_APP,750);
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    #if (BLE_APP_HID)
    // Restart Advertising
    uart_printf("dis rf mode=%x,%x\r\n",mouse_val.pair_key,sys_flag);
    if(sys_flag & FLAG_KEY_PAIRED)
    {
...
/*******************************************************************************
 * Function: app_mouse_led_handler
 * Description: app_mouse_led_ process
 * Input: msgid -Id of the message received.
 *		  param -Pointer to the parameters of the message.
 *		  dest_id -ID of the receiving task instance (TASK_GAP).
 *		  ID of the sending task instance.
...
static int app_mouse_led_handler(ke_msg_id_t const msgid,
                                          void *param,
                                          ke_task_id_t const dest_id,
                                          ke_task_id_t const src_id)
{
	...
	{
		gpio_set(LED_BT,1);
        ke_timer_clear(APP_MOUSE_LED_TIMER,TASK_APP);
...
KE_MSG_HANDLER_TAB(appm)
{
    ...
    {APP_MOUSE_LED_TIMER,	    (ke_msg_func_t)app_mouse_led_handler},
