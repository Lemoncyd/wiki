    ...
    switch(param->operation)
    {
        ...
        case (GAPM_RESET)://0
        {
            ...
            if(param->status == GAP_ERR_NO_ERROR)
            {
                ...
                cmd->role      = GAP_ROLE_CENTRAL;//GAP_ROLE_PERIPHERAL;
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    if (app_env.conidx != GAP_INVALID_CONIDX)
    {
        ...
        ke_timer_set(APP_GATTC_EXC_MTU_CMD,TASK_APP,2);
		ke_timer_set(APP_GATTC_SDP_TIMER,TASK_APP,5);
                                        ...
                                        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    //uint8_t con_dev_str_name2[] ="BK3435RC";
    ...
    if(find1 == 1 || find2 == 1)
	{
		...
		for(uint8_t index=0;index<BLE_CONNECTION_MAX;index++)
	    {
	        ...
	        if (nvds_get(NVDS_TAG_LTK2+index, &length,app_sec_env.ltk_buff) == NVDS_OK)  
	    	{
	        	...
	        	{
	        		uart_printf("delll:%d\r\n",index);
	        		//nvds_del(NVDS_TAG_LTK2+index);//之前绑定过，清除绑定信息
	        	}          
	    	}       
	    }        
                                        ...
                                        ke_task_id_t const src_id)
{
		...
		//	return (KE_MSG_CONSUMED);
 			...
 			if(app_env.role[app_env.conidx]==ROLE_MASTER)
            {  
