...
void app_sec_init()
{
	...
	uint8_t lengthk = NVDS_LEN_KEY_INDEX;
	if (nvds_get(NVDS_TAG_KEY_INDEX, &lengthk, (uint8_t *)&app_sec_env.key_index) != NVDS_OK)
    {
        // If read value is invalid, set status to not bonded
        app_sec_env.key_index= 0;
    }
#endif
...
void app_sec_remove_bond(void)
{
    ...
    if (app_sec_env.bonded == true)
    {
        ...
        #if (BLE_APP_HT)
		#if MULTI_BOND
		for(uint8_t i=0;i<app_sec_env.key_index;i++)
		{
			if (nvds_del(NVDS_TAG_LTK+app_sec_env.key_index) != NVDS_OK)
	        {
	            ASSERT_ERR(0);
	        }
		}
                                     ...
                                     ke_task_id_t const src_id)
{
    ...
    switch (param->request)
    {
        ...
        case (GAPC_LTK_EXCH):
        {
			...
			if (nvds_put(NVDS_TAG_LTK0+app_sec_env.key_index, NVDS_LEN_LTK,(uint8_t *)&cfm->data.ltk) != NVDS_OK)
            {
           		 uart_printf("%s,%d\r\n",__FUNCTION__,__LINE__);
                ASSERT_ERR(0);
            }
                                 ...
                                 ke_task_id_t const src_id)
{
    ...
    switch (param->info)
    {
        case (GAPC_PAIRING_SUCCEED):
        {
			...
			app_sec_env.key_index++;
			if(app_sec_env.key_index >= MAX_BOND_DEVICE)
			{
				app_sec_env.key_index= 0;
			}

		    #if (NVDS_SUPPORT)
            if (nvds_put(NVDS_TAG_KEY_INDEX, NVDS_LEN_KEY_INDEX,
                         (uint8_t *)&app_sec_env.key_index) != NVDS_OK)
            {
        ...
        case (GAPC_LTK_EXCH) :
        {
            ...
            if (app_env.sec_con_enabled == true)
            {
                ...
                #if MULTI_BOND
				uart_printf("%s,%d\r\n",__FUNCTION__,__LINE__);
                if (nvds_put(NVDS_TAG_LTK0+app_sec_env.key_index, NVDS_LEN_LTK,(uint8_t *)&param->data.ltk.ltk.key[0]) != NVDS_OK)
                {
                	uart_printf("%s,%d\r\n",__FUNCTION__,__LINE__);
                    ASSERT_ERR(0);
                }
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if(1)// (app_sec_env.bonded)
    {
		...
		#if MULTI_BOND
		uint8_t temp_index=app_sec_env.key_index;
		for(uint8_t i=0;i<MAX_BOND_DEVICE;i++)
		{
			...
			{
				temp_index = MAX_BOND_DEVICE-1;
			}
			
			uart_printf("keyindex=%d\r\n",temp_index);
			
			length = NVDS_LEN_LTK;
			 
			if (nvds_get(NVDS_TAG_LTK0+temp_index, &length, (uint8_t *)&ltk) == NVDS_OK)
	        {
...
static int gapc_encrypt_ind_handler(ke_msg_id_t const msgid,
                                    ...
                                    ke_task_id_t const src_id)
{
    uart_printf("encrypt_ind auth:%d\r\n",param->auth);
