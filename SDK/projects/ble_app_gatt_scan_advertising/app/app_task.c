                                      ...
                                      ke_task_id_t const src_id)
{
	...
	//ke_timer_set(APP_PERIOD_TIMER, TASK_APP, 30);
                                          ...
                                          ke_task_id_t const src_id)
{
    ...
    ke_timer_set(APP_PERIOD_TIMER, TASK_APP, scan_time);

	#else
    switch(test_scan_adv)
    {
        case 0:
        ...
        case 2:
            ...
            break;
        case 3:
            ke_timer_set(APP_PERIOD_TIMER, TASK_APP, 1000);
            appm_start_scanning();
            break;
    }
    test_scan_adv++;
...
static int gapm_adv_report_ind_handler(ke_msg_id_t const msgid, struct adv_report *param,
                                       ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    if(0x1B == param->data[3] && ((ADV_COMPID >> 0)&0x00FF) == param->data[5] && ((ADV_COMPID >> 8)&0x00FF) == param->data[6])//ADV_COMPID 
	{		
		app_get_log_u08(&param->data[0], param->data_len, "adv_dt");//CX
	}
    return KE_MSG_CONSUMED;
}
