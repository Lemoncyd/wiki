...
{
    uint8_t ch2=(ch%2)?(ch-1):(ch+1);
    uint16_t adc_data[MAX_ADC_SAMPLE_SIZE];
    for (uint8_t n = 0; n < (MAX_ADC_SAMPLE_SIZE-3); n++)
    {
        adc_data[n] = (uint16_t)(read_reg(ADC_CH_BASE + (ch2 * 0x80) + ((n+2) * 4))&0xfff);
        adc_data[n+1] = (uint16_t)((read_reg(ADC_CH_BASE + (ch2 * 0x80) + ((n+2) * 4))>>16)&0xfff);
    }
