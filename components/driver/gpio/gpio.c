...
void hal_gpio_tonggle_phy(gpio_pin_e pin)
{
    AP_GPIO->swporta_dr ^= BIT(pin);
}
...
int hal_gpio_read_all_phy(gpio_pin_e pin)
{
    uint32_t r;
...
int hal_gpio_pin_dirtype_init(gpio_pin_e pin,gpio_dir_t type){
    if(type == GPIO_OUTPUT)
    {
        AP_GPIO->swporta_ddr |= BIT(pin);
    }
...
uint32_t hal_gpio_pin_dirtype_get(gpio_pin_e pin){
    return (AP_GPIO->swporta_ddr & BIT(pin));
}
