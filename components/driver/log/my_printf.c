...
__ATTR_SECTION_SRAM__ void phy_uart0_evt_hdl(uart_Evt_t* pev)
{
    switch(pev->type)
    {
		...
		{
            osal_memcpy(&user_dt.uart_dt.urt_dt[user_dt.uart_dt.urt_len], pev->data, pev->len);
            user_dt.uart_dt.phy_len = pev->len;
            user_dt.uart_dt.urt_len += pev->len;
        } break;
...
void dbg_printf_init(void)
{
    uart_Cfg_t cfg =
    {
        ...
        .use_fifo = TRUE,
        .hw_fwctrl = FALSE,
        .use_tx_buf = FALSE,//TRUE,//FALSE,
        .parity     = FALSE,
        .evt_handler = phy_uart0_evt_hdl,//NULL,
    };
	#if 1
    hal_uart_init(cfg, UART0);//uart init
    //hal_uart_set_tx_buf(UART0, s_uart_tx_buf, TY_UART_TX_BUF_SIZE);
	#else
