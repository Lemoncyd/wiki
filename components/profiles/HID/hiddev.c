...
void hidDevPairStateCB( uint16 connHandle, uint8 state, uint8 status )
{
    ...
    else if ( state == GAPBOND_PAIRING_STATE_COMPLETE )
    {
        ...
        if ( status == SUCCESS )
        {
			...
			GAPRole_GetParameter(GAPROLE_CONN_BD_ADDR,peer_Addr);			
//			GAPRole_GetParameter(GAPROLE_BD_ADDR_TYPE,&bond_type);///????
			LOG("PAIR PEER_ADDR:");
			for(int i=0;i<6;i++)
			{
				LOG("%02X,",peer_Addr[i]);
			...
			GAPRole_SetParameter(GAPROLE_ADV_DIRECT_ADDR,6,peer_Addr);	
			GAPRole_SetParameter(GAPROLE_ADV_DIRECT_TYPE,1,0);			
			ble_pair_info.is_bond = 1;
			ble_pair_info.bond_type = 0;
			osal_memcpy( ble_pair_info.pear_addr, peer_Addr, 6 );
...
//            LOG("bond Success\n\r");
			...
			for(int i=0;i<6;i++)
			{
				LOG("%02X,",ble_pair_info.pear_addr[i]);
			}
			LOG("Addr_type:%d\n,",ble_pair_info.bond_type);
		#endif
        }
    }
