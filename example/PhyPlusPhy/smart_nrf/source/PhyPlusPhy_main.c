...
//        osal_start_timerEx(Smart_nRF_TaskID,SNRF_START_TX_EVT,1000);
	 ...
	 if(events & TEST_IO)
    {
		...
		cnt=~cnt;
		gpio_write(P31,cnt);
		VOID osal_start_timerEx(Smart_nRF_TaskID, TEST_IO, 10);
        return(events ^ TEST_IO);
    }
