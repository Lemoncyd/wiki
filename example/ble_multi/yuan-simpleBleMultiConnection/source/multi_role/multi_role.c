...
#endif
//        osal_start_timerEx(multiRole_TaskId,MULTIROLE_PERIOD_EVT,5000);
...
//					multiRole_setup_adv_scanRspData1(i);
//			}else{
//
//				for( uint8 i=0; i<MAX_CONNECTION_SLAVE_NUM; i++)
...
//
//			}
//		muliSchedule_config( MULTI_SCH_ADV_MODE, 0x21  );
...
//
        extern uint8 sch_tsk_id;
//		MultiRole_CancelConn();
        uint8 status = GAPMultiRole_TerminateConnection(0);

        muliSchedule_config( MULTI_SCH_ADV_MODE, 0x21  );
//		GAP_EndDiscoverable(sch_tsk_id);
//		HCI_LE_SetAdvEnableCmd(HCI_DISABLE_ADV);
        i = 0;
        LOG("status=%d\r\n", status);
        osal_start_timerEx(multiRole_TaskId, MULTIROLE_PERIOD_EVT, 10000);
//	}
...
//            0x06
//        };
//		LOG("GAPMultiRole_TerminateConnection\r\n");
//		extern uint8 MultiRole_CancelConn(void);
//
//		extern uint8 sch_tsk_id;
//		MultiRole_CancelConn();
