...
/*********************************************************************
    ...
    @return  none
*/
#if (defined	ADV_LDC_DIRECT)
#include "osal_snv.h"
#include "hiddev.h"
ble_pair_info_t ble_pair_info;
#endif
...
#if (defined	ADV_LDC_DIRECT)
	if(osal_snv_read(0X85, sizeof(ble_pair_info_t), &ble_pair_info ) != SUCCESS )
	{
		LOG("READ_NO_FLAG\r\n");
		ble_pair_info.is_bond = 0;		
	}
	
	if(ble_pair_info.is_bond == USER_KEY_ENTER_PAIRING)
	{
		uint8 temp_bond_count = 0;
		GAPBondMgr_GetParameter(GAPBOND_BOND_COUNT, &temp_bond_count);
		if( temp_bond_count != 0 )
		{
			if( GAPBondMgr_SetParameter(GAPBOND_ERASE_ALLBONDS, 0, NULL)  == 0 )
			{
				LOG("[bond erase ok]\r\n");
			}
			else
		...
		ble_pair_info.is_bond = 0;
		osal_snv_write( 0X85, sizeof(ble_pair_info_t), &ble_pair_info);
	}
	
	if(ble_pair_info.is_bond == 0)
	{
		// set adv type
		uint8_t param=GAP_ADTYPE_ADV_IND;
		GAPRole_SetParameter( GAPROLE_ADV_EVENT_TYPE, sizeof(uint8),&param );
		LOG("GAP_ADTYPE_ADV_IND \n");
	}
	else		
	{
		// set adv type
		uint8_t param=GAP_ADTYPE_ADV_LDC_DIRECT_IND;
		GAPRole_SetParameter( GAPROLE_ADV_EVENT_TYPE, sizeof(uint8),&param );		
		GAPRole_SetParameter(GAPROLE_ADV_DIRECT_ADDR,6,ble_pair_info.pear_addr);	
		GAPRole_SetParameter(GAPROLE_ADV_DIRECT_TYPE,1,&ble_pair_info.bond_type);
		LOG("GAP_ADTYPE_ADV_LDC_DIRECT_IND \n");				
	}
#endif
